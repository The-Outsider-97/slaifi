...
class AssetId:
    ...
    def __post_init__(self) -> None:
        ...
        if not symbol:
            raise DomainValidationError("asset symbol must not be empty")
        object.__setattr__(self, "symbol", symbol)
        if self.exchange is not None:
            exchange = self.exchange.strip().upper()
            if not exchange:
                raise DomainValidationError("exchange must not be blank when supplied")
        ...
        if self.instrument_id is not None:
            ...
            if not instrument_id:
                raise DomainValidationError("instrument_id must not be blank when supplied")
