...
class PredictionInterval:
    ...
    def __post_init__(self) -> None:
        if not isfinite(self.lower_return_rate) or not isfinite(self.upper_return_rate):
            raise ValidationError("prediction interval bounds must be finite")
        if self.lower_return_rate > self.upper_return_rate:
            raise ValidationError("prediction interval lower bound cannot exceed upper bound")
        if self.coverage_probability is not None and not (
            isfinite(self.coverage_probability)
            and 0.0 < self.coverage_probability < 1.0
        ):
            raise ValidationError("coverage_probability must be in (0, 1)")
...
class ConfidenceMeasure:
    ...
    def __post_init__(self) -> None:
        if not self.method.strip():
            raise ValidationError("confidence method must not be empty")
        if not isfinite(self.value) or not 0.0 <= self.value <= 1.0:
            raise ValidationError("confidence value must be in [0, 1]")
...
class UncertaintyMeasure:
    ...
    def __post_init__(self) -> None:
        if not self.method.strip() or not self.unit.strip():
            raise ValidationError("uncertainty method and unit must not be empty")
        if not isfinite(self.value) or self.value < 0.0:
            raise ValidationError("uncertainty value must be finite and non-negative")
...
class Prediction:
    ...
    def __post_init__(self) -> None:
        ...
        if self.input_data_at > self.generated_at:
            raise ValidationError("input_data_at cannot be later than generated_at")
        ...
        if self.expected_return_rate is None and self.expected_price is None:
            raise ValidationError("prediction requires an expected return or expected price")
        ...
        if self.expected_price is not None and self.expected_price <= 0:
            raise ValidationError("expected_price must be positive")
